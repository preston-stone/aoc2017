...
class Passphrase:
    ...
    def __init__(self):
        self.findGoodphrases()
        self.findAnagrams()
        
    def findGoodphrases(self):
        for phrase in self.passphrases:
            words = phrase.split()
            matches=[k for k,v in Counter(words).items() if v>1]

            if len(matches) == 0:
                self.goodPhraseCount += 1
                self.filteredPhrases.append(phrase)

    def findAnagrams(self):
        count = 0
        for line in self.filteredPhrases:
            words_array = list(map(lambda x: ('').join(sorted(list(x))), line.split()))
            if len(words_array) == len(set(words_array)):
                count += 1
        self.nonAnagrams = count
                
p=Passphrase()
print(p.goodPhraseCount)
print(p.nonAnagrams)